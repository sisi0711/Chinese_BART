...
def init_conversions(remove_node_adding_conversions, ud_version):
    ...
    word_nmod_compoundnn_propagation_constraint = Full(
        ...
        edges=[
            Edge(child="prop", parent="conj", label=[HasLabelFromList(["conj"])]),
            Edge(child="conj", parent="gov", label=[HasLabelFromList(["nmod:assmod", "compound:nn"])]),
        ],
    )
    ...
    def predicate_nsubj_propagation(sentence, matches, converter):
        for cur_match in matches:
            ...
            min_id = min(prop, conj)
            max_id = max(prop, conj)
            is_sen = 0
    ...
    # 并列中心语：传播修饰词
    acl_amod_propagation_constraint = Full(
        tokens=[
            Token(id="gov"),
            Token(id="prop"),
            Token(id="conj"),
        ...
        edges=[
            Edge(child="prop", parent="gov", label=[HasLabelFromList(["acl", "amod"])]),
            Edge(child="conj", parent="gov", label=[HasLabelFromList(["conj"])]),
        ],
    )

    def acl_amod_propagation(sentence, matches, converter):
        for cur_match in matches:
            gov = cur_match.token("gov")
            prop = cur_match.token("prop")
            conj = cur_match.token("conj")
    ...
    advmod_propagation_constraint = Full(
        ...
        edges=[
            Edge(child="prop", parent="gov", label=[HasLabelFromList(["advmod"])]),
            Edge(child="conj", parent="gov", label=[HasLabelFromList(["conj"])]),
        ],
    )
    ...
    passivization_alternation_constraint = Full(
        tokens=[
            ...
            Token(id="gov_nsujb", optional=True),
            Token(id="gov_mark", optional=True),
        ],
        edges=[
            Edge(child="gov_child", parent="gov", label=[HasLabelFromList(["nsubjpass", "dep"])]),
            Edge(child="gov", parent="gov_parent", label=[HasLabelFromList(["acl"])]),
            Edge(child="pass", parent="gov", label=[HasLabelFromList(["auxpass"])]),
            Edge(child="gov_nsujb", parent="gov", label=[HasLabelFromList(["nsubj"])]),
    ...
    def ccomp_propagation(sentence, matches, converter):
        for cur_match in matches:
            ...
            if sentence[prop].get_conllu_field("id") < sentence[ccomp].get_conllu_field("id"):
    ...
    def omit_nsubj_propagation(sentence, matches, converter):
        for cur_match in matches:
            ...
            if (conj_id < punct_id and prop_id > punct_id and gov_id > punct_id) or (conj_id > punct_id and prop_id < punct_id and gov_id < punct_id):
                ...
                is_diff_sen = 0
            cur_iid = conj
            # print(sentence[conj].get_conllu_field("upos"), conj_id, gov_id, prop_id, punct_id, cur_iid)
            if is_conj_nsubj == 0 and is_diff_sen:
                if dobj != -1:
    ...
    def omit_dobj_propagation(sentence, matches, converter):
        for cur_match in matches:
            ...
            if (conj_id < punct_id and prop_id > punct_id and gov_id > punct_id) or (conj_id > punct_id and prop_id < punct_id and gov_id < punct_id):
                ...
                is_diff_sen = 0
            cur_iid = conj
            if is_conj_dobj == 0 and is_diff_sen:
                if nsubj != -1:
    ...
    conversion_list = [
        ...
        Conversion(ConvTypes.BART, acl_amod_propagation_constraint, acl_amod_propagation),
